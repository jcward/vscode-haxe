...
HaxeContext.prototype = {
			...
			if(dlt >= this.configuration.haxeDiagnosticDelay) {
				if(this.client.isPatchAvailable) {
					...
					while(_g1 < _g) {
						...
						if(document != null) this.diagnostics["delete"](document.uri);
					...
					while( $it0.hasNext() ) {
						...
						if(ds2.document != null && ds2.lastModification > ds2.lastSave) {
							if(!(ds2.lastSave < ds2.saveStartAt)) {
								...
								this.undiagnoseDocument(ds2.document);
	...
	,initTmpDir: function() {
		...
		if(this.tmpDir == null) this.tmpDir = "";
		if(this.tmpDir == "auto") try {
	...
	,removeTmpFile: function(ds) {
		if(this.useTmpDir && ds.tmpPath != null && ds.tmpPath != ds.realPath) js_node_Fs.unlinkSync(ds.tmpPath);
		ds.tmpPath = null;
	}
	...
	,onSaveDocument: function(document,saveFromVS) {
		if(saveFromVS == null) saveFromVS = false;
		var path = document.uri.fsPath;
		var ds = this.getDocumentState(path,document);
		ds.lastSave = new Date().getTime();
		if(saveFromVS) this.removeTmpFile(ds);
		if(ds.diagnoseOnSave) this.removeAndDiagnoseDocument(document); else if(document != null) this.diagnostics["delete"](document.uri);
