...
var HaxeContext = function(context) {
	...
	this.diagnosticStartTime = 0;
	this.maxLastDiagnoseTime = 1;
...
HaxeContext.isSaving = function(ds) {
	return ds.saveStart > ds.lastSave;
};
HaxeContext.needDiagnostic = function(ds) {
	return ds.lastSave > ds.lastDiagnostic;
};
HaxeContext.prototype = {
	get_diagnosticRunning: function() {
		return this.diagnosticStartTime > this.maxLastDiagnoseTime;
	}
	,check: function() {
		var time = new Date().getTime();
		if(this.checkDiagnostic && this.diagnosticStartTime <= this.maxLastDiagnoseTime) {
			if(time - this.maxLastDiagnoseTime >= this.configuration.haxeDiagnosticDelay) {
				if(this.client.isPatchAvailable) {
					...
					while(tmp.hasNext()) {
						...
						if(ds.lastModification > ds.lastSave) {
							if(ds.saveStart <= ds.lastSave) {
								isDirty = true;
								ds.saveStart = new Date().getTime();
								ds.diagnoseOnSave = false;
								document.save();
							}
						} else if(!needDiagnose) needDiagnose = ds.lastSave > ds.lastDiagnostic; else needDiagnose = true;
					}
					if(!isDirty) {
						this.checkDiagnostic = false;
						if(needDiagnose) this.diagnose(1);
					}
				}
			}
		}
	...
	,applyDiagnostics: function(message) {
		...
		while(tmp.hasNext()) {
			...
			this.diagnostics["delete"](ds.document.uri);
		}
		...
		while(_g < _g1.length) {
			...
			if(__map_reserved[key] != null) tmp1 = all.getReserved(key); else tmp1 = all.h[key];
			var diags = tmp1;
			if(diags == null) {
				diags = [];
		...
		var tmp2 = all.keys();
		while(tmp2.hasNext()) {
			var fileName = tmp2.next();
			var diags2 = __map_reserved[fileName] != null?all.getReserved(fileName):all.h[fileName];
			fileName = js_node_Path.normalize(fileName);
			var url = Vscode.Uri.file(fileName);
			if(diags2 == null) {
				this.diagnostics.set(url,[]);
				continue;
			}
			this.diagnostics.set(url,diags2);
			this.getDocumentState(url.fsPath).lastDiagnostic = t;
		}
		if(t > this.maxLastDiagnoseTime) this.maxLastDiagnoseTime = t;
	}
	,getTime: function() {
	...
	,getDocumentState: function(path) {
		...
		ds = { path : path, lastModification : 0, lastSave : new Date().getTime(), saveStart : 0, lastDiagnostic : 0, document : null, diagnoseOnSave : this.diagnoseOnSave};
	...
	,onOpenDocument: function(document) {
		...
		ds.lastSave = ds.lastModification = new Date().getTime();
		this.removeAndDiagnoseDocument(document);
	}
	,onSaveDocument: function(document) {
		var ds = this.getDocumentState(document.uri.fsPath);
		ds.lastSave = new Date().getTime();
		if(ds.diagnoseOnSave) this.removeAndDiagnoseDocument(document); else ds.diagnoseOnSave = this.diagnoseOnSave;
	}
	,diagnose: function(trying) {
		var _g = this;
		this.diagnosticStartTime = new Date().getTime();
		...
		this.client.sendAll(function(s,message,err) {
			if(err != null) {
				if(trying <= 0) {
					...
					_g.maxLastDiagnoseTime = new Date().getTime();
	...
	,changePatchs: function(events) {
		...
		if(changed) this.client.sendAll(function(s,message,error) {
			...
			if(document1.languageId != "haxe") return;
			var changes1 = events[events.length - 1].contentChanges;
			if(changes1.length == 0) return;
			var lastChange = changes1[changes1.length - 1];
			var cursor = editor.selection.active;
			var text2 = document1.lineAt(cursor).text;
			var char_pos2 = cursor.character - 1;
			...
			if(lastLen > 0) {
				if(HaxeContext.reWS.match(insertText.charAt(lastLen - 1))) {
					var ei = char_pos2 + 1;
					while(ei < len) {
						if(!HaxeContext.reWS.match(text2.charAt(ei))) break;
						++ei;
					}
					this.checkDiagnostic = ei < len;
				} else this.checkDiagnostic = true;
				return;
			}
			if(lastChange.rangeLength > 0) {
				var ei2 = char_pos2;
				while(ei2 < len) {
					if(!HaxeContext.reWS.match(text2.charAt(ei2))) break;
					++ei2;
				}
				this.checkDiagnostic = ei2 < len;
				return;
			}
		}
	}
		...
		return new Promise(function(resolve,reject) {
			...
			make_request = function() {
				...
				client.sendAll(function(s,message,err) {
					...
					} else resolve(_g.parse_items(message));
				},true);
			};
			...
			var doRequest = function() {
				...
				if(isPatchAvailable) changeDebouncer.whenDone(function() {
					...
					document.save().then(function(saved) {
						if(cancelToken.isCancellationRequested || !saved) reject([]); else if(saved) make_request1();
					});
				} else make_request1();
			};
